...
// function getballTime(jawheelTime) {
// return (rotationsTime < parseInt(jawheelTime) ? temptime = rotationsTime + 's' : temptime = (rotationsTime - jawheelTime) + 's');
// }
function ballrotateTo(deg, jawheelTime) {
  ...
  $(ballbg).playKeyframe({
    ...
    complete: function () {
      ...
      socket.emit('rouletteStopEvent', { "playerId": userId }, function (res) {
      });
...
function getCurreyData() {
  socket.emit("getCurrecyMasterData", (res) => {
    window.CurrencyMaster = res.data;
  });
}
/*End:  End Game */

// create- Start : betStopNumberHistory 
function betStopNumberHistory() {
  console.log(' --> betStopNumberHistory');
  socket.emit('BetHistory', { "userId": userId }, function (res) {
    if (res.status == 'success') {
      ...
      for (let i = 0; i < res.data.betStopeNumber.length; i++) {
        // Red or Black
        if (numred.includes(parseInt(res.data.betStopeNumber[i]))) {
          redBlack += '<tr><td>' + res.data.betStopeNumber[i] + '</td><td></td></tr>'
        } else {
          redBlack += '<tr><td></td><td>' + res.data.betStopeNumber[i] + '</td></tr>'
        }

        // Even or Odd
        if (res.data.betStopeNumber[i] % 2 == 0) {
          evenOdd += '<tr><td>' + res.data.betStopeNumber[i] + '</td><td></td></tr>'
        } else {
          evenOdd += '<tr><td></td><td>' + res.data.betStopeNumber[i] + '</td></tr>'
        }

        // 1-18 to 19-36
        if (first1to18.includes(parseInt(res.data.betStopeNumber[i]))) {
          betweenNumber += '<tr><td>' + res.data.betStopeNumber[i] + '</td><td></td></tr>'
        } else {
          betweenNumber += '<tr><td></td><td>' + res.data.betStopeNumber[i] + '</td></tr>'
        }

        // Left Midd Right
        if (left.includes(parseInt(res.data.betStopeNumber[i]))) {
          leftMiddRight += '<tr><td>' + res.data.betStopeNumber[i] + '</td><td></td><td></td></tr>'
        } else if (right.includes(parseInt(res.data.betStopeNumber[i]))) {
          leftMiddRight += '<tr><td></td><td></td><td>' + res.data.betStopeNumber[i] + '</td></tr>'
        } else {
          leftMiddRight += '<tr><td></td><td>' + res.data.betStopeNumber[i] + '</td><td></td></tr>'
        }

        // 1st12 2nd12 3rd12
        if (firstTwelve.includes(parseInt(res.data.betStopeNumber[i]))) {
          betweenNumberTwelve += '<tr><td>' + res.data.betStopeNumber[i] + '</td><td></td><td></td></tr>'
        } else if (secondTwelve.includes(parseInt(res.data.betStopeNumber[i]))) {
          betweenNumberTwelve += '<tr><td></td><td>' + res.data.betStopeNumber[i] + '</td><td></td></tr>'
        } else {
          betweenNumberTwelve += '<tr><td></td><td></td><td>' + res.data.betStopeNumber[i] + '</td></tr>'
        }
...
// End me: betStopNumberHistory

//Start lighting Roulette page refresh
function lightingrouletRefreshteGame() {
  socket.emit('ReconnectGame', { "playerId": userId }, function (res) {
    if (res.status == "success") {
      ...
      if (res.data.game_status == "started") {
        ...
        if (betHi.length > 0) {
          ...
          resetBoard();
          resetGameLobby();
          if (localStorage.playerPlaceBetObj) {
            var getOlddata = JSON.parse(localStorage.playerPlaceBetObj)
...
$('.2xBets').click(function () {
  ...
  localStorage.setItem("betsDetail", JSON.stringify(previousBetDetails));

  // return false;
  var previousBetDetails2 = localStorage.getItem("betsDetail");
  console.log('previousBetDetails2 -->', previousBetDetails2);
...
$('.reBets').click(function () {
  ...
  console.log('previousBetDetails -->', previousBetDetails);
