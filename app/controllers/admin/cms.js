...
module.exports = function (model, config) {
	...
	module.save = async function (request, response) {
		var inputData = {
			...
			isShowOnScreen: request.body.isShowOnScreen
		};
		try {
	...
	module.edit = async function (request, response) {
		...
		if (cmsId != "" && cmsId != 0) {
			try {
				var cmsDetail = await model.Cms.findById(cmsId);
				if (cmsDetail != null) {
	...
	module.update = async function (request, response) {
		...
		if (cmsId != "" && cmsId != 0) {
			try {
				var cmsData = await model.Cms.findById(cmsId);
				if (cmsData != null) {
	...
	module.delete = async function (request, response) {
		...
		if (cmsId != "" && cmsId != 0) {
			try {
				var cmsData = await model.Cms.destroy({ where: { id: cmsId } });
